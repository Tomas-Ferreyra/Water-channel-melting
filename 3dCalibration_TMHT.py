...
im1 = imageio.imread(
    '/Volumes/Ice blocks/Scan water channel/25-08-07/Camera back/DSC_0433.NEF')
im2 = imageio.imread(
    '/Volumes/Ice blocks/Scan water channel/25-08-07/Camera back/DSC_0434.NEF')
...
def order_points(centsx, centsy, max_iter=100, dist_close=4, dist_long=10):
    puntos = np.hstack((np.array([centsx]).T, np.array(
        [centsy]).T, np.array([[0]*len(centsy)]).T))
    mima = [np.argmax(puntos[:, 0]+puntos[:, 1]),
            np.argmax(puntos[:, 0]-puntos[:, 1])]
    ...
    while len(puntos) > 0 and itere < max_iter:
        ...
        for i in range(len(puntos)):
            p3 = puntos[i]
            dist = np.linalg.norm(np.cross(p2-p1, p1-p3)
...
vid1 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/Camera 1/DSC_0456.MOV', 'ffmpeg')  # 7548 frames
vid2 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/Camera 1/DSC_0457.MOV', 'ffmpeg')  # 7464 frames
...
for n in tqdm(range(14)):
    ...
    if n in [0, 1, 2, 3, 4, 5, 6, 7, 9]:
        ...
        rrr = remove_small_objects(
            imf/img > 2.6, min_size=5000, connectivity=1)
...
for i in tqdm(range(14)):
    ...
    if i < 10:
        bob = blob_log((1-coso), threshold=0.03, min_sigma=1,
                       max_sigma=1.2, num_sigma=3)
    else:
        bob = blob_log((1-coso), threshold=0.04, min_sigma=1,
                       max_sigma=1.2, num_sigma=3, overlap=0.0)
    ...
    orpu, xgr, ygr = order_points(
        cx, cy, max_iter=107, dist_close=4, dist_long=20)
...
lal = least_squares(residual_firstz, [
                    0, 1, 1000, 1, 0, 1000, 0, 0], method='lm', xtol=1e-12, max_nfev=10000)
...
# %%
cutoffs = [0, 2781, 5562, 8343, 11124, 13905, 16686,
           19467, 22248, 25029, 27810, 30590, 33371, 36152, 38934]
...
vidc1 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/Camera 1/DSC_0468.MOV', 'ffmpeg')  # 7272 frames
vidc2 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/Camera 1/DSC_0469.MOV', 'ffmpeg')  # 7272 frames

vidu1 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/Camera 1/DSC_0456.MOV', 'ffmpeg')  # 7548 frames
vidu2 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/Camera 1/DSC_0457.MOV', 'ffmpeg')  # 7548 frames
...
for i in tqdm(range(2)):
    ...
    if i == 0:
        ...
        masks.append((fals > 210) * (fals < 250) * (fals != 211)
                     * (fals != 213) * (fals != 216) * (fals != 222))
...
for i in tqdm(range(2)):
    ...
    if i == 0:
        bob = blob_log((1-coso), threshold=0.05, min_sigma=1,
                       max_sigma=1.2, num_sigma=3)
    else:
        bob = blob_log((1-coso), threshold=0.034, min_sigma=1,
                       max_sigma=1.2, num_sigma=3, overlap=0.0)
    pont = np.zeros_like(coso)
    edgem = masks[i]*1. - binary_erosion(masks[i], disk(1))*1.
    edy, edx = np.where(edgem)
    ...
    if i == 0:
        orpu, xgr, ygr = order_points(
            cx, cy, max_iter=107, dist_close=4, dist_long=10)
    else:
        orpu, xgr, ygr = order_points(
            cx, cy, max_iter=107, dist_close=4, dist_long=30)
...
ex1 = np.array([ellipse(55, 20, x1s[i], y1s[i]+180)[0]
               for i in range(8)]).flatten()
ey1 = np.array([ellipse(55, 20, x1s[i], y1s[i]+180)[1]
               for i in range(8)]).flatten()
ex2 = np.array([ellipse(57, 21, x2s[i], y2s[i]+180)[0]
               for i in range(8)]).flatten()
ey2 = np.array([ellipse(57, 21, x2s[i], y2s[i]+180)[1]
               for i in range(8)]).flatten()
...
def recal(v, x, y, order=1):
    if order == 1:
        ...
        bt = v[2] * x + v[5] * y + 1
    if order == 2:
        xt = v[0] * x + v[3] * y + v[6] + v[8] * \
            x*y + v[11] * x**2 + v[14] * y**2
        yt = v[1] * x + v[4] * y + v[7] + v[9] * \
            x*y + v[12] * x**2 + v[15] * y**2
        bt = v[2] * x + v[5] * y + 1 + v[10] * \
            x*y + v[13] * x**2 + v[16] * y**2
    X, Y = xt/bt, yt/bt
    return X, Y
...
def func2(v):
    xt = v[0] * ax1 + v[3] * ay1 + v[6] + v[8] * \
        ax1*ay1 + v[11] * ax1**2 + v[14] * ay1**2
    yt = v[1] * ax1 + v[4] * ay1 + v[7] + v[9] * \
        ax1*ay1 + v[12] * ax1**2 + v[15] * ay1**2
    bt = v[2] * ax1 + v[5] * ay1 + 1 + v[10] * \
        ax1*ay1 + v[13] * ax1**2 + v[16] * ay1**2
    X, Y = xt/bt, yt/bt
    return (X-ax2)**2+(Y-ay2)**2
...
# Labels for each picked point in order
labels = ["(0,0)", "(1,0)", "(0,1)"]

def pick_points_on_figure(x, y, fig_num):
    picked_points = []
...
for i in range(num_figures):
    ...
    x = np.random.rand(20)
    y = np.random.rand(20)
    points = pick_points_on_figure(x, y, i)
    all_picked_points.append(np.array(points))

for i, points in enumerate(all_picked_points):
    print(f"Figure {i + 1} picked points: {points}")
...
vidd1 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/25-08-07/Camera down/DSC_9522.MOV', 'ffmpeg')  # 8055 frames
vidd2 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/25-08-07/Camera down/DSC_9523.MOV', 'ffmpeg')  # 4593 frames

vidu1 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/25-08-07/Camera up/DSC_6009.MOV', 'ffmpeg')  # 8100 frames
vidu2 = imageio.get_reader(
    '/Volumes/Ice blocks/Scan water channel/25-08-07/Camera up/DSC_6010.MOV', 'ffmpeg')  # 4548 frames
...
xmedd1, ymedd1, iesd1 = position_grid(
    vidd1, backd, -20, 8054, in_frame=0, lims=[None, None, 1200, 2450])
xmedd2, ymedd2, iesd2 = position_grid(
    vidd2, backd, -20, 4592, in_frame=0, lims=[None, None, 1200, 2450])
iesd2 += 8054
xmd, ymd, ied = np.hstack((xmedd1, xmedd2)), np.hstack(
    (ymedd1, ymedd2)), np.hstack((iesd1, iesd2))

xmedu1, ymedu1, iesu1 = position_grid(
    vidu1, backu, -100, 8099, in_frame=0, lims=[130, None, 1200, 2450])
xmedu2, ymedu2, iesu2 = position_grid(
    vidu2, backu, -100, 4547, in_frame=0, lims=[130, None, 1200, 2450])
iesu2 += 8099
xmu, ymu, ieu = np.hstack((xmedu1, xmedu2)), np.hstack(
    (ymedu1, ymedu2)), np.hstack((iesu1, iesu2))
...
# %%
framesd = np.array([540, 1100, 1840, 2380, 2920, 3630, 4550,
                   5290, 6090, 6790, 7420, 8074, 8814, 9764])
framesu = np.array([500, 1050, 1800, 2330, 2870, 3590, 4500,
                   5240, 6040, 6740, 7370, 8030, 8769, 9729])
...
for i in tqdm(range(14)):
    ...
    outd, gridposd = find_indices(
        pp, known, eps=0.2, lim_ind=[0, 25, -1e5, 1e5])
...
for i in [0, 13]:
    ...
    pos2 = np.zeros((55, 3))
    pos2[:, 0], pos2[:, 1], pos2[:,
                                 2] = xw, np.arange(-30, 294+1, 6), (13-i)*10
